...
class OpenAILLM:
    ...
    async def fetch_completion(
        self, chat: List[Dict[str, str]], temperature: float | None = None
    ) -> str:
    ...
    async def generate_async(
        self, chats: List[List[Dict[str, str]]], temperature=None
    ) -> List[str]:
    ...
    def generate(self, chats: List[List[Dict[str, str]]]) -> List[str]:
        for attempt in range(10):
            try:
                ...
                logger.warning(
                    f"Error during OpenAI API call: {e}. Retrying... (attempt {attempt + 1}/10)"
                )
        raise RuntimeError(
            "Failed to get response from OpenAI API after multiple attempts."
        )
...
class OpenAIEmbedding:
    ...
    def embed(self, texts: List[str], column_name: str = "default"):
        ...
        if column_name not in self.embeddings_map or len(
            self.embeddings_map[column_name]
        ) < len(texts):
            self.embeddings_map[column_name] = LRUCache(
                capacity=max(10000, len(texts) + 100)
            )

        new_texts = [
            text for text in set(texts) if text not in self.embeddings_map[column_name]
        ]
        logger.debug(
