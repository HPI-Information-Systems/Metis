...
def split_mixed_column(col: pd.Series, colname: str) -> pd.DataFrame:
    ...
    df_out[f"{colname}_num"] = numeric_values.fillna(0)  # Fill NaNs with 0 for numeric values
...
def split_datetime_column(col: pd.Series, colname: str) -> pd.DataFrame:
    ...
    df_out[f"{colname}_str"] = (df_out[f"{colname}_num"].isnull() & ~col.isnull()).astype(int)
...
def encode_dataset(
    ...
    for column in types.keys():
        if types[column] == "categorical":
            ...
            ohe_features = pd.DataFrame(encoded.toarray().astype(ohe_dtype), columns=feature_names)
            df_detect = pd.concat([df_detect.drop(columns=[column]), ohe_features], axis=1)
            ...
            numeric_features[f"{column}_num"] = scaler.fit_transform(numeric_features[[f"{column}_num"]])
            df_detect = pd.concat([df_detect.drop(columns=[column]), numeric_features], axis=1)
            ...
            if column in embeddings:
                ...
                emb_df = pd.DataFrame(emb, columns=[f"{column}_emb{i}" for i in range(emb.shape[1])])
                df_detect = pd.concat([df_detect.drop(columns=[column]), emb_df], axis=1)
            else:
