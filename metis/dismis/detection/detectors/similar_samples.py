    ...
    def __call__(self, dataset: pd.DataFrame, types: Dict[str, str], target_columns: List[str] = None, embeddings: Dict[str, pd.DataFrame] = {}) -> Tuple[pd.DataFrame, pd.DataFrame, Dict[str, float]]:
        ...
        for idx, column in enumerate(df_detect.columns):
            if df_detect[column].dtype == 'object':
                try:
                    df_detect[column] = scaler.fit_transform(df_detect[column].astype(float).values.reshape(-1, 1))
                except (ValueError, TypeError):
                    df_detect[column] = df_detect[column].astype('category').cat.codes
            else:
                df_detect[column] = scaler.fit_transform(df_detect[column].values.reshape(-1, 1))
        times['preprocessing'] = time.time() - preprocessing_starttime

        columns = df_detect.columns.tolist() if target_columns is None else target_columns
        for target_column in columns:
            ...
            tree = KDTree(df_detect.drop(columns=[target_column]).iloc[target_idx, :].values)
            times['fitting'] += time.time() - fitting_starttime

            querying_starttime = time.time()
            _, ind = tree.query(df_detect.drop(columns=[target_column]).iloc[target_idx, :], k=self.num_neighbors+1)
            times['querying'] += time.time() - querying_starttime

            scoring_starttime = time.time()
            differences = abs(target[ind][:, 1:] - np.repeat(target.reshape(-1, 1), self.num_neighbors, axis=1))

            scores = np.mean(differences, axis=1)
            df_score.iloc[target_idx, idx] = scores
            assessed.append(target_column)

            predictions = (scores > np.percentile(scores, 99)).astype(int)  # Example threshold
...
class FAISSSimilarSamplesDetector(DMVDetector):
    ...
    def __call__(self, dataset: pd.DataFrame, types: Dict[str, str], target_columns: List[str] = None, embeddings: Dict[str, pd.DataFrame] = {}) -> Tuple[pd.DataFrame, pd.DataFrame, Dict[str, float]]:
        ...
        df_score = pd.DataFrame(np.zeros(dataset.shape, dtype=np.float32), columns=dataset.columns, index=dataset.index)
        df_predict = pd.DataFrame(np.zeros(dataset.shape, dtype=np.int32), columns=dataset.columns, index=dataset.index)
        ...
        # === Detection Loop ===
        columns = ["_".join(col.split("_")[:-1]) for col in df_detect.columns] if target_columns is None else target_columns
        for target_column in columns:
            if types[target_column] not in self.target_types:
                continue
            #print(target_column)
            encoded_target_columns = [col for col in df_detect.columns if target_column == "_".join(col.split("_")[:-1])]
            ...
            if self.use_correlations and not len(df_detect.columns) > 500:
                ...
                for col in encoded_target_columns:
                    ...
                    for other_col, item in corr[col].items():
                        if abs(item) >= median_corr and other_col not in encoded_target_columns:
            ...
            target_array = df_detect[encoded_target_columns].values  # Already float32 from encode_dataset
            ...
            if self.use_correlations and not len(df_detect.columns) > 500:
                ...
                feature_cols = [c for c in correlated_columns if c not in encoded_target_columns]
            ...
            # === Querying ===
            querying_starttime = time.time()
            distances, ind = index.search(X, self.num_neighbors + 1)  # +1 because the closest is the point itself
            times['querying'] += time.time() - querying_starttime
            ...
            for batch_idx in range(n_batches):
                ...
                if n_target_dims == 1:
                    ...
                    neighbor_values = target_array[batch_ind]  # shape: (batch, k, n_target_dims)
                    batch_target_exp = batch_target[:, np.newaxis, :]  # shape: (batch, 1, n_target_dims)
                    differences = np.abs(neighbor_values - batch_target_exp)  # broadcasting, no tile needed

                    dist_max = batch_distances.max()
                    if dist_max > 0:
                        neighbor_similarities = 1 - (batch_distances / dist_max)  # shape: (batch, k)
                    else:
                        neighbor_similarities = np.ones_like(batch_distances)

                    # Weighted mean: expand similarities for broadcasting
                    weighted_diff = differences * neighbor_similarities[:, :, np.newaxis]
                    batch_scores = weighted_diff.mean(axis=(1, 2)).astype(np.float32)
                #scores = (differences).mean(axis=1)
                batch_scores[dataset[target_column].iloc[start:end].isnull()] = 0  # Set scores to 0 for null values in the target column
                scores[start:end] = batch_scores

                batch_predictions = (batch_scores > np.percentile(batch_scores, 98)).astype(int)
...
class FAISSNoDubSimilarSamplesDetector(DMVDetector):
    ...
    def __call__(self, dataset: pd.DataFrame, types: Dict[str, str], target_columns: List[str] = None, embeddings: Dict[str, pd.DataFrame] = {}) -> Tuple[pd.DataFrame, pd.DataFrame, Dict[str, float]]:
        ...
        for target_column in columns:
            ...
            feature_mask = ~df_detect.columns.isin(encoded_target_columns)
            X = df_detect.loc[:, feature_mask].values  # Already float32, avoid drop() copy
            ...
            # === Querying ===
            querying_starttime = time.time()
            fetch_k = self.num_neighbors * self.fetch_multiplier if types[target_column] != "text" else self.num_neighbors * 2
            distances, ind = index.search(X, fetch_k + 1)  # +1 because the closest is the point itself
            times['querying'] += time.time() - querying_starttime
            ...
            for batch_idx in range(n_batches):
                ...
                mask = (target_array[batch_ind] == repeated_target.reshape(batch_target.shape[0], -1, batch_target.shape[1])).sum(axis = 2) != batch_target.shape[1]
                ...
                neighbor_distances = np.repeat(batch_distances, batch_target.shape[1], axis=1)
                neighbor_similarities = 1 - neighbor_distances
                neighbor_similarities[~mask2] = 0
                neighbor_similarities_normalized = neighbor_similarities / neighbor_similarities.sum(axis=1, keepdims=True)
                differences = np.abs(neighbor_values - repeated_target)
                batch_scores = (neighbor_similarities_normalized * differences).sum(axis=1)
...
class MultiSimilarSamplesDetector(DMVDetector):
    ...
    def __call__(self, dataset: pd.DataFrame, types: Dict[str, str],
        ...
        df_score_template = pd.DataFrame(np.zeros(dataset.shape), columns=dataset.columns)
        df_predict_template = pd.DataFrame(np.zeros(dataset.shape, dtype=int), columns=dataset.columns)
        ...
        columns = ["_".join(col.split("_")[:-1]) for col in df_detect.columns] \
        ...
        for target_column in columns:
            ...
            encoded_target_columns = [col for col in df_detect.columns
            ...
            X_all = df_detect.drop(columns=encoded_target_columns).values.astype('float32')
            X_corr = df_detect.drop(columns=encoded_target_columns)[correlated_columns].values.astype('float32') \
                     if correlated_columns is not None else None
            ...
            for k in self.num_neighbors_list:
                ...
                fetch_k = k * self.fetch_multiplier if types[target_column] != "text" else k * 2
            ...
            for config in configs:
                ...
                scores = self._compute_scores(
                    ...
                    n_samples=n_samples,
                    fetch_k=ind.shape[1] - 1  # Pass actual fetch_k for no_duplicate variants
                )

                config['df_score'].iloc[:, idx] = scores
                threshold = np.percentile(scores, 98) if config['remove_dups'] else np.percentile(scores, 98)
    ...
    def _compute_scores(self, target_array, distances, ind, num_neighbors,
                       remove_duplicates, dataset, target_column, n_samples, fetch_k=None):
        ...
        for batch_idx in range(n_batches):
            ...
            if remove_duplicates:
                ...
                batch_scores = self._score_no_duplicates(
                    batch_target, target_array, batch_ind, batch_distances, num_neighbors, actual_fetch_k
                )
            else:
    ...
    def _score_no_duplicates(self, batch_target, target_array, batch_ind, batch_distances, num_neighbors, fetch_k):
        ...
        repeated_target_reshaped = repeated_target.reshape(batch_target.shape[0], -1, batch_target.shape[1])
        neighbor_values_reshaped = target_array[batch_ind]
        mask = (neighbor_values_reshaped == repeated_target_reshaped).sum(axis=2) != batch_target.shape[1]
        ...
        neighbor_distances_flat = np.repeat(batch_distances, target_array.shape[1], axis=1)
