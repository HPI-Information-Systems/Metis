...
class MemoryDataset(Dataset):
    ...
    def __init__(self, dataset_dictionary):
        ...
        self.dataframe = dataset_dictionary["dirty"].copy().astype(str).map(self.value_normalizer)
        self.has_ground_truth = True
        self.clean_dataframe = dataset_dictionary["clean"].copy().astype(str).map(self.value_normalizer)
...
class RAHADetector(DMVDetector):
    ...
    def __call__(self, dataset: pd.DataFrame, types: Dict[str, str], target_columns: List[str] = None, embeddings: Dict[str, pd.DataFrame] = {}) -> Tuple[pd.DataFrame, pd.DataFrame, Dict[str, float]]:
        ...
        if len(df_detect.columns) != len(self.clean_dataset.columns):
            ...
            raise ValueError("The number of columns in the dirty dataset does not match the number of columns in the clean dataset.")
