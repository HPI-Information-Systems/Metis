...
def series_hash(s: pd.Series) -> str:
    ...
    return hashlib.sha1(
        pd.util.hash_pandas_object(s, index=True).values
    ).hexdigest()
...
class ScikitPosthocsESDDetector:
    ...
    def fit(self, X):
        ...
        result = sp.outliers_gesd(X, alpha=self.alpha, hypo=False, outliers=self.max_outliers)
...
class PyODDetector(DMVDetector):
    def __init__(self, detector_name: str, target_types: List[str] = ["numeric", "categorical", "text"], **kwargs):
        """
    ...
    def __call__(self, dataset: pd.DataFrame, types: Dict[str, str], target_columns: List[str] = None, embeddings: Dict[str, pd.DataFrame] = {}) -> Tuple[pd.DataFrame, pd.DataFrame, Dict[str, float]]:
        ...
        for column in columns:
            ...
            target = df_detect[column].dropna() if self.detector_name == "MAD" else df_detect[column]
...
class PyODDetector2(DMVDetector):
    def __init__(self, detector_name: str, target_types: List[str] = ["numeric", "categorical", "text"], nanvalue=0, **kwargs):
        """
        Initialize the PYODDetector with a specific detector.
    ...
    def __call__(self, dataset: pd.DataFrame, types: Dict[str, str], target_columns: List[str] = None, embeddings: Dict[str, pd.DataFrame] = {}) -> Tuple[pd.DataFrame, pd.DataFrame, Dict[str, float]]:
        ...
        for column in columns:
            if types[column] not in self.target_types:
                continue
            extraction_starttime = time.time()
            target_values = self._extract_features(df_detect[column], types[column], embeddings)
            times['extraction'] += time.time() - extraction_starttime
    ...
    for row_idx, value in enumerate(column):
        ...
        if len(value_str) >= substring_length:
            ...
            for offset in range(substring_length):
                for i in range(offset, len(value_str) - substring_length + 1, substring_length):
...
class RepeatedSubstringOutlierDetectorOD(PyODDetector):
    ...
    def _extract_features(self, column: pd.Series, type: str = None, embeddings: Dict[str, pd.DataFrame] = None) -> np.ndarray:
        """
        Extract features based on the maximum number of repeated substrings of a given length.
...
class SemanticDetector(FeatureDetector):
    def __init__(self, LLM, target_types, targets: List[str] | Dict = None, invert=False):
        ...
        self.invert = invert
        self.target_types = target_types
        self.targets = targets if targets is not None else ["Missing", "Unknown", "Placeholder", "?"]
    ...
    def _extract_features(self, column: pd.Series, type: str, embeddings: Dict[str, pd.DataFrame]) -> np.ndarray:
        ...
        if isinstance(self.targets, dict):
            if isinstance(self.targets[column.name][0], str):
                target_emb = np.array(self.embedding_model.embed(self.targets[column.name]), dtype=np.float32)
            elif isinstance(self.targets[column.name][0], list) and isinstance(self.targets[column.name][0][0], float):
                target_emb = np.array(self.targets[column.name], dtype=np.float32)
            else:
                raise ValueError(f"Unsupported target format for column {column.name}: {self.targets[column.name]}")
        else:
            if isinstance(self.targets[0], str):
                target_emb = np.array(self.embedding_model.embed(self.targets), dtype=np.float32)
            elif isinstance(self.targets[0], list) and isinstance(self.targets[0][0], float):
                target_emb = np.array(self.targets, dtype=np.float32)
            else:
                raise ValueError(f"Unsupported target format: {self.targets}")

        # Normalize embeddings to unit vectors
        target_emb = target_emb / (np.linalg.norm(target_emb, axis=1, keepdims=True) + 1e-8)
...
class MultiSemanticOutlierDetectorNew(DMVDetector):
    ...
    """
    def __init__(self, LLM, target_types, num_neighbors_list=[3, 10, 25, 100], remove_duplicates=False):
        self.embedding_model = LLM
        self.target_types = target_types
