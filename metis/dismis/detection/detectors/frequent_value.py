...
def mean_neighbor_diff2(x, n):
    ...
    avg_diffs = np.divide(
        total_diffs, counts, out=np.zeros_like(total_diffs), where=counts > 0
    )
    ...
    def __call__(
        ...
        for target_column in target_columns:
            ...
            if len(npvals) < 2:
                continue
            is_peak = (
                npvals[1:-1] > (npvals[:-2] + self.relative_prominence * value_range)
            ) & (npvals[1:-1] > (npvals[2:] + self.relative_prominence * value_range))
            first_peak = npvals[0] > (
                npvals[1] + self.relative_prominence * value_range
            )
            last_peak = npvals[-1] > (
                npvals[-2] + self.relative_prominence * value_range
            )
            is_peak = np.concatenate(([first_peak], is_peak, [last_peak]))
            detection_labels = (
                df_detect[target_column]
                .dropna()
                .isin(values.index.to_numpy()[is_peak])
                .astype(int)
                .to_numpy()
            )

            differences = mean_neighbor_diff(npvals, self.num_neighbors) / (
                value_range + 1e-5
            )
...
class FrequentValuesDetector2(DMVDetector):
    ...
    def __call__(
        ...
        for target_column in target_columns:
            ...
            differences = mean_neighbor_diff2(npvals, self.num_neighbors) / (
                value_range + 1e-5
            )
