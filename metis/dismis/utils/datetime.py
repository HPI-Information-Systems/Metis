...
def datetime_to_numeric(series: pd.Series) -> Tuple[pd.Series, Optional[datetime], str]:
    ...
    if unit == "Y":
        numeric = dt_series.apply(
            lambda x: x.year - min_dt.year if not pd.isnull(x) else np.nan
        )
    elif unit == "M":
        numeric = dt_series.apply(
            lambda x: (
                (x.year - min_dt.year) * 12 + (x.month - min_dt.month)
                if not pd.isnull(x)
                else np.nan
            )
        )
        ...
        numeric = dt_series.apply(
            lambda x: x.toordinal() - min_ordinal if not pd.isnull(x) else np.nan
        )
    elif unit == "h":
        # Total hours
        numeric = dt_series.apply(
            lambda x: (
                (x - min_dt).total_seconds() / 3600 if not pd.isnull(x) else np.nan
            )
        )
    elif unit == "min":
        # Total minutes
...
def numeric_to_datetime(
    numeric_series: pd.Series, min_datetime: datetime, unit: str
) -> pd.Series:
    """
